...
if (!isset($_GET['cultivation_id'])) {
    echo "<p class='text-red-600'>No cultivation selected.</p>";
    exit;
}

$cultivation_id = intval($_GET['cultivation_id']);

// Fetch stage records
$stmt = $pdo->prepare("SELECT * FROM landuse.paddy_stages WHERE cultivation_id = :cid ORDER BY applied_date ASC");
$stmt->execute(["cid" => $cultivation_id]);
$stages = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>

<div class="mt-6">
    <h3 class="text-lg font-semibold mb-2">Stage Data (Paddy Inputs)</h3>

    <?php if (empty($stages)): ?>
        <p class="text-gray-600">No stage inputs recorded.</p>
        <button id="addStageBtn" class="mt-2 px-4 py-2 bg-green-600 text-white rounded shadow hover:bg-green-700">Add Stage Information</button>
    <?php else: ?>
        <button id="addStageBtn2" class="mb-2 px-4 py-2 bg-green-600 text-white rounded shadow hover:bg-green-700">Add New Stage</button>
